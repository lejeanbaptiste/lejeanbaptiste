...
- Added a typecheck gate to CI: every workspace now has a `typecheck` script (`tsc --noEmit -p tsconfig.json`), run from the root via `npm run typecheck --workspaces --if-present` in the same job as `lint`. `cwrc-leafwriter` gets a small wrapper (`packages/cwrc-leafwriter/scripts/typecheck.mjs`) instead of the bare command, since its isolated `tsc` run legitimately reports ~27 errors under `../../apps/commons/` — the confirmed `@src/*` cross-package-alias artifacts noted above, not real problems (its actual build is webpack, which never runs bare `tsc`). The wrapper filters those out by path prefix and fails on anything else, so the gate stays meaningful instead of either being permanently red or silently ignoring that workspace. Verified the gate both passes cleanly on the current baseline and actually fails (confirmed exit code + real error surfaced) when a genuine type error is introduced.
