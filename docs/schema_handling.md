...
**CBETA P5 is a bundled catalog entry** — `id: 'cbeta'`, shown in the schema wizard's "more" tier. Unlike every other catalog schema it is **not downloaded**: `apps/desktop/resources/schema/cbeta_p5.rng` + `cbeta_p5.sch` + `cbeta.css` ship inside the app (electron-builder `schema-catalog` extra resource) and `installCatalogSchema` copies them into the project's `schema/` folder, writing `catalogId: 'cbeta'`. The `.rng` is the plugin's Grognard-loosened build (`ljb-cbeta-loosen v2`): the NE inventory is added to `tei_model.phrase`, `@ref`/`@key` land on the authority elements, `<date>` carries the Sanmiao parse children, and (v2) `tei_div` matches both `<cb:div>` and TEI `<div>` while `<creation>`/`<keywords>`/`<author>` are relaxed — so the CBETA, Daozang, Kanripo, Wikisource and BDRC importers can all target it, and the standard **Tag Appearance** highlighter (right rail → CSS tab, `schema/tag-colors.css`) works exactly as for TEI. `catalogId: 'cbeta'` also maps to the standard TEI edition-metadata field set (`getMetadataFieldsForCatalog`) — including the mandatory `profileDesc/langUsage/language`, without which project onboarding cannot complete (the metadata dialog would render no source-language picker). `scripts/check-cbeta-schema-bundle.mjs` (run in `npm run typecheck`) guards the bundled `.rng` against shipping stale, un-loosened, or pre-v2, and keeps its Sanmiao date parts in step with `sanmiaoSchemaMerge.ts`. Bundled schemas are refreshed on app update, never re-downloaded — the update checker skips them.
...
- **TEI:** `teiHeader/encodingDesc/appInfo/application[@ident="grognard"]` with encoder `name` and `date/@when` (last save only — does not modify `revisionDesc`)
- **Orlando:** `REVISIONDESC/RESPONSIBILITY[@RESP="Grognard"]` with `DATE/@when` and encoder name (does not modify other `RESPONSIBILITY` entries)
