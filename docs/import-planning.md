...
- **xml (v1)** — same-family TEI/Orlando only: demote `@key` to `@ana` (`grognard-former-key:`), attach project schema PIs, merge edition metadata, provenance in `revisionDesc/change`. Skip `entities.xml`. Element remapping still deferred.
