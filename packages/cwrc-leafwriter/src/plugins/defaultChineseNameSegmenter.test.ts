...
describe('defaultChineseNameSegmenter', () => {
  it('splits a common single-character surname from a 2-character given name', () => {
    const result = defaultChineseNameSegmenter({ name: '周世雄', projectLang: 'zh-Hant', romanize });
  ...
  it('splits a known compound surname as one unit', () => {
    const result = defaultChineseNameSegmenter({ name: '歐陽修', projectLang: 'zh-Hant', romanize });
  ...
  it('returns null for names outside the plausible 2-4 character range', () => {
    expect(defaultChineseNameSegmenter({ name: '周', projectLang: 'zh-Hant', romanize })).toBeNull();
    expect(
