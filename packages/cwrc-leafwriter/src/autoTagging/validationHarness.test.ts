...
const fakeSuggestion = (
  over: Partial<Suggestion> & { surface: string; occurrence: number },
): Suggestion => ({
  id: 'x',
  source: 'ai',
  action: 'add',
...
describe('scoreSuggestions', () => {
  ...
  it('counts a spurious prediction (no matching gold span) as a false positive', () => {
    const predicted = [
      ...gold.map((g) => fakeSuggestion({ ...g })),
      fakeSuggestion({ surface: '虛構', occurrence: 1, tag: 'persName' }),
    ];
    const report = scoreSuggestions(gold, predicted);
  ...
  it('counts a right-span-wrong-tag prediction against both tags and reports it separately', () => {
    ...
    expect(report.wrongTag).toEqual([
      { surface: '張衡', occurrence: 1, goldTag: 'persName', predictedTag: 'placeName' },
    ]);
  });

  it('treats an empty gold and empty prediction as perfect (no evidence of failure)', () => {
    expect(scoreSuggestions([], []).overall).toMatchObject({
      tp: 0,
      fp: 0,
      fn: 0,
      precision: 1,
