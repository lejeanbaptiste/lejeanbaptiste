...
const packIds = (
  process.env.AUTHORITY_LIVE_PACKS ?? 'cbdb-offices,cbdb-places,chgis-places,dila-places'
)
  .split(',')
  .map((s) => s.trim())
...
describe('authority tag bomb -> AI validate calibration (opt-in live)', () => {
  maybe(
    'scores AI validate cleaning up real tag-bomb output vs hand-tagged gold',
    async () => {
      ...
      const sameSpanLines = report.sameSpanGroups.map((g) => {
        const optionsStr = g.options
          ...
          .join(' vs ');
        const verdict =
          g.rankedCorrectHighest === null
            ? 'n/a (no correct option)'
            : g.rankedCorrectHighest
              ? 'RANKED CORRECTLY'
              : 'MISRANKED';
