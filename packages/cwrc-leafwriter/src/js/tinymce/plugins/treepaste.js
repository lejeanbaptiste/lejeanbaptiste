  ...
  tinymce.create('tinymce.plugins.TreePastePlugin', {
    init: function (ed, url) {
      ...
      function fragmentElements(fragmentDocument) {
        return Array.prototype.slice
          .call(fragmentDocument.documentElement.childNodes)
      ...
      function buildEditorFragmentFromXml(xml) {
        ...
        Array.prototype.slice
          .call(parsed.document.documentElement.childNodes)
      ...
      function buildModeContent(mode, text, originalHtml) {
        ...
        if (mode === 'paragraphs') {
          return {
            content: pasteSpecial.textToParagraphHtml(text, blockTag, true) || originalHtml,
          };
        }
        ...
        if (mode === 'plain') {
          return {
            content:
              pasteSpecial.textToParagraphHtml(text, blockTag) || pasteSpecial.escapeHtml(text),
          };
        }
      ...
      function choosePasteMode(ambiguity, text, originalHtml, done) {
        ...
        var modes = [
          {
            mode: 'paragraphs',
            label: 'Paragraphs',
            description: 'Blank lines create new paragraphs.',
          },
          {
            mode: 'line-breaks',
            label: 'Line breaks',
        ...
        if (
          modes.some(function (option) {
            return option.mode === selectedMode && option.disabled;
          })
        ) {
        ...
        function updateSelection(nextMode) {
          var option = modes.find(function (item) {
            return item.mode === nextMode && !item.disabled;
          });
          if (!option) return;
        ...
        $dialog.on('keydown', function (event) {
          var available = modes.filter(function (option) {
            return !option.disabled;
          });
          var index = available.findIndex(function (option) {
    ...
    _preProcess: function (pl, o) {
      ...
      if (
        ...
        /<(h[1-6r]|p|div|address|pre|form|table|tbody|thead|tfoot|th|tr|td|li|ol|ul|caption|blockquote|center|dl|dt|dd|dir|fieldset)/.test(
          o.content,
        )
      ) {
        // IE9 adds BRs before/after block elements when contents is pasted from word or for example another browser
        ...
        if (type) {
          ...
          each(dom.select('span', p), function (span) {
            ...
            if (type == 'ul' && /^__MCE_ITEM__[\u2022\u00b7\u00a7\u00d8o\u25CF]/.test(html))
              dom.remove(span);
            else if (/^__MCE_ITEM__[\s\S]*\w+\.(&nbsp;|\u00a0)*\s*/.test(html)) dom.remove(span);
          });
