...
export const loadValidator = async ({ state }) => {
...
export const validate = async ({ state }) => {
    const writer = window.writer;
    if (!writer || !state.validator.hasWorkerValidator)
        return;
    const workerValidator = window.leafwriterValidator;
    if (!workerValidator.hasValidator())
        return;
    const documentString = await writer.converter.getDocumentContent(false);
    const validationProgress = ({ partDone, state, valid, errors }) => {
        if (state <= 2) {
...
export const getAt = async ({ actions }, { action, attributeName, index, parentXpath, tagName, xpath, }) => {
    const { getTagAt, getElementsForTagAt, getAttributesForTagAt, getTagAttributeAt, getValuesForTagAttributeAt, } = actions.validator;
    switch (action) {
        case 'TagAt':
            if (!tagName || !parentXpath)
                return;
...
export const getTagAt = async ({ state }, { tagName, parentXpath, index }) => {
    if (!state.validator.hasWorkerValidator)
        return;
    const workerValidator = window.leafwriterValidator;
...
export const getElementsForTagAt = async ({ state }, { xpath, index }) => {
    if (!state.validator.hasWorkerValidator)
        return;
    const workerValidator = window.leafwriterValidator;
...
export const getAttributesForTagAt = async ({ state }, { xpath, index }) => {
    if (!state.validator.hasWorkerValidator)
        return;
    const workerValidator = window.leafwriterValidator;
...
export const getTagAttributeAt = async ({ state }, { attributeName, parentXpath }) => {
    if (!state.validator.hasWorkerValidator)
        return;
    const workerValidator = window.leafwriterValidator;
...
export const getValuesForTagAttributeAt = async ({ state }, { xpath }) => {
    if (!state.validator.hasWorkerValidator)
        return;
    const workerValidator = window.leafwriterValidator;
...
export const getValidTagsAt = async ({ state }, params) => {
    if (!state.validator.hasWorkerValidator)
        return;
    const workerValidator = window.leafwriterValidator;
