...
export const useContextmenu = (writer, contextMenuState) => {
    ...
    const context = contextMenuState ? { ...ui.contextMenu } : { show: false };
    const selectionOverlapNodes = (_rng) => {
    ...
    const getEntitiesOptions = () => {
        ...
        const options = [...entityMappings].map(([key, { label }]) => {
    ...
    const paramsForAddTag = () => {
        if (!writer || !context.rng || !context.element)
            return;
    ...
    const paramsForChangeTag = () => {
        if (!writer || !context.element || !context.element.parentNode)
            return;
        const parentNode = context.element.parentNode;
        ...
        const request = { xpath, index, selection };
        return request;
    };
    const paramsForAddTagBefore = () => {
        ...
        if (containerIndex < 0)
            containerIndex = 0;
        const selection = { type: 'before', xpath, containerIndex };
        const request = { xpath, index, selection };
        return request;
    };
    ...
    const getTagsFor = async (params) => {
        const tags = await actions.validator.getValidTagsAt(params);
        if (!tags)
            return [];
        return tags;
    };
    const getTagItems = (tags, action) => {
        if (!writer)
            return;
        const menu = tags.map(({ name, fullName }) => {
            return {
                id: uuidv4(),
                type: 'tag',
                displayName: name,
                ...
                onClick: () => {
                    if (context.tagId === undefined)
                        return log.warn('No Tag selected');
        ...
        initialize: async () => {
            ...
            context.element =
                context.rng.commonAncestorContainer.nodeType === Node.ELEMENT_NODE
                    ? context.rng.commonAncestorContainer // ?double-check
