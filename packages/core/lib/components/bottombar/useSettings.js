...
const useSettings = () => {
    ...
    return {
        editorModeShouldChange: (editorMode) => {
            ...
            if (editorMode === 'xml' && writer.mode !== writer.XML) {
                ...
                if (writer.mode !== writer.XMLRDF || writer.allowOverlap === true)
                    doModeChange = true;
            }
            else if (editorMode === 'xmlrdfoverlap') {
                if (writer.mode !== writer.XMLRDF || writer.allowOverlap === false)
                    doModeChange = true;
            }
            else if (editorMode === 'rdf') {
                if (writer.mode !== writer.RDF || writer.allowOverlap === false)
                    doModeChange = true;
            }
            if (!doModeChange)
                return [false, null];
      ...
      `;
        ...
        changeEditorMode: ({ mode, isUndo }) => {
            ...
            if (editorMode)
                message = `${message} to ${editorMode.label}`;
            return message;
        },
        changeAnnotationMode: ({ mode, isUndo }) => {
            const annotationMode = editor.setAnnotationrMode(mode);
            let message = isUndo ? 'Annotation Mode restored' : 'Annotation Mode has changed';
            if (annotationMode)
                message = `${message} to ${annotationMode.label}`;
            return message;
        },
        schemaShouldChange: async (schemaId) => {
            const writer = window.writer;
            if (!writer)
                return [false, { type: 'error', text: 'writer not found' }];
        ...
        changeSchema: ({ schemaId, isUndo }) => {
            ...
            if (schema)
                message = `${message} to ${schema.name}`;
            return message;
        },
    };
};
