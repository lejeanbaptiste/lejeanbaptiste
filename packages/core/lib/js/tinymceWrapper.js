...
export const tinymceWrapperInit = function ({ writer, editorId, layoutContainerId, buttons1, buttons2, buttons3, }) {
            ...
            editor.on('init', (event) => {
                ...
                editor.on('contextmenu', (event) => {
                    ...
                    if (writer.isReadOnly)
                        return;
                    const editorPosition = writer.utilities.getOffsetPosition(editor.getContentAreaContainer(), window.document.documentElement);
    ...
    // tinymce handlers
    const fireNodeChange = (element) => {
        // fire the onNodeChange event
        const parents = [];
        writer.editor?.dom.getParent(element, (node) => {
    ...
    const onNodeChangeHandler = (event) => {
        if (!writer.editor)
            return;
        ...
        if (element.nodeType !== 1) {
            ...
            if (element.getAttribute('id') === 'mcepastebin')
                return;
            if (element.getAttribute('_tag') === null &&
                ...
                if (element.getAttribute('data-mce-bogus') !== null) {
                    ...
                    if (rng.collapsed) {
                        ...
                        if (writer.editor.lastKeyPress === 'Home' ||
    ...
    const doHighlightCheck = (event, _ev) => {
        if (!writer.editor)
            return;
        ...
        if (parent.nodeType === Node.ELEMENT_NODE && parent.hasAttribute('_entity')) {
            ...
            if ((writer.editor?.dom.hasClass(parent, 'start') && event.code === 'ArrowLeft') ||
        ...
        if (!id)
            return;
        if (id === writer.entitiesManager.getCurrentEntity())
            return;
