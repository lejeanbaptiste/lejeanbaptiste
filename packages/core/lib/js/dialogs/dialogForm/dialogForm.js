...
class DialogForm {
    ...
    writer;
    $el;
    showConfig; // the config object sent to the dialog's "show" method
    cwrcWriter; // reference to the cwrcWriter if this is a note form
    // set to false to cancel saving
    isValid = true;
    type;
    mode = null;
    currentId = null; // ADD or EDIT
    currentData = {
        attributes: {},
        properties: {},
        customValues: {},
    ...
    attributesWidget;
    attWidgetInit = false;
    static processForm(dialogInstance) {
    ...
    static populateForm(dialogInstance) {
        const data = dialogInstance.currentData;
        $('[data-type]', dialogInstance.$el)
            ...
            if (type === 'attributes') {
                ...
                switch (type) {
                    case 'select':
                        ...
                        if (!selectedOption[0])
                            value = 'other'; //if there is no option for the value, select 'other' option
                        formEl.val(value);
                        //@ts-ignore
                    ...
                    case 'radio':
                        ...
                        if (!radioOption[0])
                            radioOption = $('input[value="other"]', formEl); //if there is no option for the value, check 'other' option
    ...
    initAttributeWidget(dialogInstance, config) {
    ...
    show(config = {}) {
        ...
        if (this.attributesWidget) {
            if (!this.attWidgetInit)
                this.initAttributeWidget(this, config);
        ...
        if (this.mode === DialogForm.ADD) {
            ...
            mappedProps?.forEach((propName) => {
                ...
                const propMapping = this.writer.schemaManager.mapper.getAttributeForProperty(this.type, propName);
