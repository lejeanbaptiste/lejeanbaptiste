...
class DateDialog {
    ...
    `).appendTo(parentEl);
        ...
        this.dialog.$el.on('beforeSave', (event, dialog) => {
            const type = $(`#${id}_type input:checked`).val();
            let error = false;
            if (type === 'date' || type === 'DATE' || type === 'DATESTRUCT') {
                ...
                if (Array.isArray(endString))
                    endString = endString[0];
                if (typeof startString === 'number')
                    startString = startString.toString();
                if (typeof endString === 'number')
                    endString = endString.toString();
    ...
    dateTypeField(id) {
      ...
      >
        ...
        ${options
            ...
            //exception
            if (this.schemaMappingMatch('cwrcEntry') && value === 'DATESTRUCT')
                return '';
            return `
            <input
              type="radio"
