...
class NoteDialog {
    ...
    `;
        ...
        //dialog events
        this.dialog.$el.on('buildDynamicFields', (event, config, dialog) => {
        ...
        optionsTypeElement.change((event) => {
            if (this.typeAtt?.choices)
                return;
        ...
        noteOtherTypeElement.on('keyup', (event) => {
            if (event.code === 'Space') {
    ...
    toggleOtherTypeTextField = (show) => {
        this.dialog.$el.find(`#${this.id}_noteOtherTypeSlot`).toggle(show);
        if (!show)
            this.dialog.$el.find(`#${this.id}_noteOtherType`).val('');
    };
    setTypeOptions = () => {
...
//# sourceMappingURL=NoteDialog.js.map