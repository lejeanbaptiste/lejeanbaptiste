  ...
  `;
    ...
    <div class="annotationDialog">
      <div class="schemaHelp" />
        ...
        let langOptions = iso6392.reduce((result, lang) => {
    ...
    show() {
        ...
        const hasResp = this.writer.schemaManager.isAttributeValidForTag(this.respAttribute, this.tagName);
        hasResp ? $resp.parent().show() : $resp.parent().hide();
        $resp.prop('checked', false);
        let firstLang = $(`#${this.id}_lang > option:eq(0)`).val();
        if (!firstLang)
            return;
        if (Array.isArray(firstLang))
            firstLang = firstLang[0];
        if (typeof firstLang === 'number')
            firstLang = firstLang.toString();
