...
class AttributesEditor {
    ...
    toggleHintComponent = (show) => {
        if (!show) {
            const attributesUsed = $('.attsContainer div.attribute', this.$schemaDialog).filter((i, li) => $(li).is(':visible') == true);
    ...
    doShow({ attributes, tagFullname, tagName, tagPath, }) {
        ...
        if (this.writer.mode !== this.writer.XML) {
            ...
            if (type)
                this.$schemaDialog.find('.entityWarning').show();
        }
...
//# sourceMappingURL=attributesEditor.js.map