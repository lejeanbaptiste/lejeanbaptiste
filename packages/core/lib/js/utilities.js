...
class Utilities {
     ...
     */
    convertTextForExport(text, isAttributeValue = false) {
    ...
    getPreviousTextNode(node, skipWhitespace = false) {
    ...
    getNextTextNode(node, skipWhitespace = false) {
        const walker = node.ownerDocument?.createTreeWalker(node.ownerDocument, NodeFilter.SHOW_TEXT, {
            acceptNode: (node) => {
                // whitespace match does not include \uFEFF since we use that to prevent empty tags
                if (skipWhitespace === false ||
     ...
     */
    selectElementById = (id, selectContentsOnly = false) => {
     ...
     * @returns {String|null}
     */
    getElementXPath(element, tagAttribute) {
     ...
     */
    evaluateXPath(contextNode, xpath) {
        ...
        let nsResolver = null;
        const defaultNamespace = doc instanceof Document ? doc.documentElement.getAttribute('xmlns') : null;
     ...
     */
    processArray(array, processFunc, refreshRate = 250) {
        //@ts-ignore
