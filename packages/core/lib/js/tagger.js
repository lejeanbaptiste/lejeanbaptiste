...
class Tagger {
    ...
    NO_COMMON_PARENT = 'no_common_parent';
    OVERLAP = 'overlap';
    VALID = 'valid';
    constructor(writer) {
        this.writer = writer;
    ...
    getCurrentTag(id) {
        if (!id)
            return $(this.writer.editor?.selection.getNode());
     ...
     */
    setAttributesForTag(tag, attributes) {
        ...
        for (const attName in attributes) {
            ...
            //   continue;
            // }
            if (!RESERVED_ATTRIBUTES.has(attName))
                continue;
     ...
     * @param {Object} attributes A name/value map of attributes
     */
    addAttributesToTag(tag, attributes) {
     ...
     */
    removeAttributeFromTag(tag, attributeName) {
    ...
    editTagDialog(id) {
        ...
        if (Array.isArray(id))
            id = id[0];
        const tag = this.getCurrentTag(id);
        if (tag.attr('_entity')) {
            this.writer.editor.currentBookmark = this.writer.editor.selection.getBookmark(1);
            ...
            if (attrID) {
                ...
                if (entry)
                    this.writer.dialogManager.show(entry.getType(), { entry });
            }
            else {
            ...
            if (attributesEditor) {
                attributesEditor.show({
                    ...
                    callback: (newAttributes) => {
                        if (newAttributes)
                            this.editStructureTag(tag, newAttributes);
                    },
                    tagName,
                    tagPath,
                });
    ...
    changeTagDialog(tagName, id) {
        ...
        if (tag.attr('_entity')) {
            this.writer.dialogManager.confirm({
                ...
                callback: (confirmed) => {
                    ...
                    if (!newTag)
                        return log.warn('new tag not found');
                    if (!newTag.parentElement)
                        return log.warn('new tag parentElement not found');
                    ...
                    if (attributesEditor) {
                        attributesEditor.show({
                            ...
                            callback: (newAttributes) => {
                                if (newAttributes)
                                    this.editStructureTag($(newTag), newAttributes, tagName);
                            },
                            tagName,
                            tagPath,
                        });
        ...
        if (attributesEditor) {
            attributesEditor.show({
                ...
                callback: (newAttributes) => {
                    if (newAttributes)
                        this.editStructureTag(tag, newAttributes, tagName);
                },
                tagName,
                tagPath,
            });
     ...
     */
    addEntityDialog(type, tag) {
        if (!this.writer.editor)
            return;
        const requiresSelection = this.writer.schemaManager.mapper.doesEntityRequireSelection(type);
        const result = !requiresSelection && this.writer.editor.selection.isCollapsed()
            ? this.VALID
     ...
     */
    processNewContent(domContent) {
        const processNewNodes = (currNode, direction) => {
            if (currNode.nodeType === Node.ELEMENT_NODE) {
                const currNodeElement = currNode;
    ...
    finalizeEntity(type, info) {
        ...
        if (type === 'note')
            delete info.attributes.otherType; //remove otherType attribute;
    ...
    editEntity(id, info) {
        ...
        const removeEntity = isNamedEntity && (uriAttribute && info.attributes[uriAttribute]) === undefined;
        if (removeEntity) {
    ...
    addEntityTag(entity, range) {
        ...
        if (entity.isNote()) {
            ...
            // }
            const tag = this.writer.editor?.dom.create('span', $.extend(tagAttributes, {
                _entity: true,
                _note: true,
                _tag: parentTag,
                _type: type,
            ...
            if (range.startContainer.parentNode !== range.endContainer.parentNode) {
                const nodes = this.getNodesInBetween(range.startContainer, range.endContainer, NodeFilter.SHOW_TEXT);
                ...
                const start = this.writer.editor?.dom.create('span', {
                    _entity: true,
                    _type: type,
          ...
          `);
                ...
                const end = this.writer.editor?.dom.create('span', {
                    _entity: true,
                    _type: type,
                ...
                const start = this.writer.editor?.dom.create('span', $.extend(tagAttributes, {
                    _entity: true,
                    _tag: parentTag,
                    _type: type,
                    class: `entity ${type} start end`,
     ...
     */
    editStructureTag(tag, attributes, tagName) {
        this.sanitizeObject({ obj: attributes, isAttributes: true });
     ...
     */
    removeStructureTag(id, removeContents) {
        //? what to do if id is multiple
        if (Array.isArray(id))
            id = id[0];
        ...
        if (invalidDelete) {
            this.showInvalidDeleteConfirm(tag[0], false, (confirmed) => {
                if (confirmed)
                    doRemove();
            });
            return;
    ...
    removeStructureTagContents(id) {
        ...
        if (invalidDelete) {
            this.showInvalidDeleteConfirm(tag[0], true, (confirmed) => {
                if (confirmed)
                    doRemove();
            });
            return;
     ...
     */
    processRemovedContent(domContent, processChildren = true) {
     ...
     */
    sanitizeObject({ obj, isAttributes }) {
    ...
    showInvalidDeleteConfirm(element, isContents, callback) {
     ...
     * @returns {Integer}
     */
    isSelectionValid({ isStructTag, cleanRange, }) {
        const sel = this.writer.editor?.selection;
        // disallow empty entities
        if (!isStructTag && sel.isCollapsed())
     ...
     * @returns {jQuery}
     */
    getCorrespondingEntityTags(tag) {
        tag = $(tag);
        if (tag.hasClass('start') && tag.hasClass('end'))
            return tag;
        const boundaryType = tag.hasClass('start') ? 'end' : 'start';
        const currentNode = tag[0];
        const nodeId = currentNode.getAttribute('name');
        const walker = currentNode.ownerDocument.createTreeWalker(currentNode.ownerDocument, NodeFilter.SHOW_ELEMENT, {
            acceptNode: (node) => {
     ...
     */
    getNodesInBetween(start, end, filter = NodeFilter.SHOW_ALL) {
        const nodes = [];
