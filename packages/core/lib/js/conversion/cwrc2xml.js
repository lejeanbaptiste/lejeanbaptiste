...
class CWRC2XML {
     ...
     */
    async getDocumentContent(includeRDF) {
        if (includeRDF && this.writer.mode === this.writer.XML)
            includeRDF = false;
        // remove highlights
        ...
        const entities = [];
        this.writer.entitiesManager.eachEntity((id, entity) => entities.push(entity));
     ...
     * @returns {String}
     */
    buildXMLString(node, identifyEntities = false) {
        const _this = this;
        let xmlString = '';
        function _nodeToStringArray(currNode) {
            ...
            if (isEntity && id) {
                ...
                const isEmpty = currNode[0].childNodes.length === 0 ||
    ...
    prepareText(body) {
        ...
        function recursiveTextConversion(parentNode) {
        ...
        $(overlappingEntIds).each((index, id) => {
            ...
            entry.setRange(_this.getRangesForEntity(id));
     ...
     */
    getRangesForEntity(entityId) {
        // const _this = this;
        const getOffsetFromParentForEntity = (id, parent, isEnd) => {
            let offset = 0;
        ...
        const doRangeGet = ($element, isEnd) => {
     ...
     * @returns Array
     */
    getNodeOffsetsFromParent(parent) {
        const _this = this;
        let currentOffset = 0;
        const offsets = [];
    ...
    //! deprecated?
    determineOffsetRelationships(offsets) {
        const entityOffsets = [];
        const relationships = {};
        for (let i = 0; i < offsets.length; i++) {
            const o = offsets[i];
...
//# sourceMappingURL=cwrc2xml.js.map