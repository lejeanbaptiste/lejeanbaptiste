...
class XML2CWRC {
     ...
     */
    processDocument(doc, schemaIdOverride) {
        // clear current doc
     ...
     * @returns {String}
     */
    buildEditorString(node, includeComments) {
        let editorString = '';
        const doBuild = (node) => {
     ...
     * @returns {Boolean}
     */
    isLegacyDocument(doc) {
        const hasRdf = $(doc).find('rdf\\:RDF, RDF').length > 0;
        const hasOldAnnotationIds = $(doc).find('*[annotationId], *[offsetId]').length > 0;
        const hasOldRdfParent = this.writer.utilities.evaluateXPath(doc, this.writer.schemaManager.mapper.getRdfParentSelector()) === null;
    ...
    processRDF(doc) {
        ...
        $rdfs.children().each((index, el) => {
            ...
            if (!isOverlapping) {
                ...
                const mappingInfo = this.writer.schemaManager.mapper.getReverseMapping(entityEl, true);
                if (mappingInfo.type !== entityConfig.type) {
                    log.warn(`xml2cwrc.processRDF: entity type mismatch. RDF = ${entityConfig.type}. Element = ${mappingInfo.type}.`);
                }
        ...
        const rdfParentXpath = this.writer.utilities.evaluateXPath(doc, this.writer.schemaManager.mapper.getRdfParentSelector());
        let rdfParent = rdfParentXpath &&
        ...
        if (rdfParent?.length === 1) {
            ...
            while (currNode !== this.writer.schemaManager.getHeader() &&
    ...
    buildEditorStringDeferred(parentNode) {
        //@ts-ignore
        ...
        const processNode = (nodeData, prevDepth) => {
            ...
            if (depth <= prevDepth) {
                let stackEntry = closingStack.pop();
                if (stackEntry) {
    ...
    buildDocumentAndInsertEntities(doc) {
        //@ts-ignore
        const dfd = new $.Deferred();
    ...
    insertEntities() {
        ...
        const docRoot = $(`[_tag="${this.writer.schemaManager.getRoot()}"]`, this.writer.editor.getBody())[0];
        // insert entities
        const insertEntity = (entry) => {
            let startNode = null;
            let endNode = null;
            let startOffset = 0;
            let endOffset = 0;
    ...
    getTextNodeFromParentAndOffset(parent, offset) {
        let currentOffset = 0;
        let textNode = null;
        const getTextNode = (parent) => {
            let ret = true;
            $(parent)
                .contents()
                //@ts-ignore
...
//# sourceMappingURL=xml2cwrc.js.map