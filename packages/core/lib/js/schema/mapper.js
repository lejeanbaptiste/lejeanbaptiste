...
export const getAttributeString = (attObj) => {
    let str = '';
 ...
 * @returns {String}
 */
export const getTagAndDefaultAttributes = (entity, closeTag = true) => {
 ...
 * @returns {Array}
 */
export const getDefaultMapping = (entity) => {
    return [getTagAndDefaultAttributes(entity), `</${entity.getTag()}>`];
};
export const xmlToString = (xmlData) => {
...
class Mapper {
    ...
    getMappings() {
        ...
        if (!mappings)
            throw new Error('Schema mappings not found');
        return mappings;
    }
    /**
     ...
     */
    cleanupMappings({ entityElement, mappingInfo, isCWRC, textTag, shouldRemoveMatch, }) {
        const isTextTag = (node) => {
        ...
        const removeMatch = (match) => {
            switch (match.nodeType) {
                ...
                case Node.TEXT_NODE:
                    ...
                    break;
                default:
                    log.warn(`mapper.getReverseMapping.cleanupMappings: cannot remove node with unknown type ${match}`);
                    break;
            }
        };
    ...
    getValueFromXPath(contextEl, xpath) {
        let value;
        let result = this.writer.utilities.evaluateXPath(contextEl, xpath);
        if (result === null)
            return;
        if (result.nodeType) {
            ...
            value = result;
            let innerXPath = /^local-name\((.*)\)$/.exec(xpath); // try to get the inside of the name function
            if (innerXPath) {
                ...
                if (!innerResult) {
                    log.warn(`mapper.getReverseMapping.getValueFromXPath: cannot get match for unrecognizable xpath ${xpath}`);
                    result = undefined;
                }
                else {
     ...
     */
    getReverseMapping(element, cleanUp = false) {
     ...
     */
    getMappingForProperty(type, property) {
     ...
     * @returns {String|undefined} The mapping
     */
    getAttributeForProperty(type, property) {
     ...
     * @param {Entity} entity
     */
    updatePropertiesFromAttributes(entity) {
        const type = entity.getType();
        const entry = this.getMappings().entities.get(type);
        if (!entry?.mapping)
            return;
     ...
     * @returns {Boolean}
     */
    doesEntityRequireSelection(type) {
        if (!type)
            return false;
     ...
     */
    convertTagToEntity(tag, showEntityDialog = false) {
        ...
        if (isNote)
            this.writer.tagger.addNoteWrapper(tag, entityType);
        const entity = this.writer.entitiesManager.addEntity(config);
        if (showEntityDialog) {
     ...
     */
    findEntities(typesToFind = ['person', 'place', 'date', 'org', 'organization', 'title', 'link']) {
        // const allTypes = [
        //   'person',
        //   'place',
        //   'date',
        ...
        for (const [type, entity] of entityMappings.entries()) {
            if (typesToFind.length !== 0 || typesToFind.indexOf(type) !== -1) {
                ...
                const matches = $(entityTagNames.join(','), this.writer.editor?.getBody()).filter((index, element) => {
                    if (element.getAttribute('_entity') === 'true')
                        return false;
                    if ($(element).parents(`[_tag="${headerTag}"]`).length !== 0)
                        return false;
