...
export const setSchemaElements = (elements) => {
 ...
 * @returns {Array}
 */
export const getParentsForTag = (tag) => {
 ...
 * @returns {Array}
 */
export const getChildrenForTag = (tag) => {
    const elements = getEntriesForTag(tag);
    if (elements.length == 0) {
        log.warn(`schemaNavigator: cannot find element for ${tag}`);
        return [];
 ...
 * @returns {Array}
 */
export const getAttributesForTag = (tag) => {
    const elements = getEntriesForTag(tag);
    if (elements.length === 0) {
        // log.warn('schemaNavigator: cannot find element for '+tag);
        return [];
 ...
 * @returns {Array}
 */
export const getAttributesForPath = (path) => {
    const element = getEntryForPath(path);
    if (element === null) {
        log.warn(`schemaNavigator: cannot find element for ${path}`);
        return [];
 ...
 * @returns {Array}
 */
const getEntriesForTag = (name) => {
    const matches = [];
    queryDown(schemaJSON.grammar, (item) => {
 ...
 * @returns {Array}
 */
const getElementChildren = (element, type) => {
    let children = [];
 ...
 */
const getParentsJSON = (defName, defHits, level, parents) => {
 ...
 */
const getChildrenJSON = (currEl, defHits, level, type, children, refParentProps) => {
    // first get the direct types
    let hits = [];
    queryDown(currEl, (item) => {
        if (item.$key === 'element' && item !== currEl)
 ...
 */
const queryDown = (context, matchingFunc, processRefs = false) => {
    let continueQuery = true;
    const defHits = {};
 ...
 * @returns {String}
 */
const getFullNameFromDocumentation = (documentation) => {
...
//# sourceMappingURL=schemaNavigator.js.map