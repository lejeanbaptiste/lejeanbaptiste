...
class LayoutManager {
    ...
    init(config) {
        if (config.modules) {
            ...
            const regions = Object.entries(config.modules).forEach(([region, modules]) => {
                this.modulesLayout.set(region, modules);
            });
        }
        this.$containerid = this.writer.getUniqueId('cwrc_');
        this.$container = $(`<div id="${this.$containerid}" class="cwrc cwrcWrapper" />`).appendTo(config.container);
        if (config.name)
    ...
    `;
        ...
        if (South)
            html += this.addPanel('south', South);
        html += '</div>';
        //EAST - INSIDE CENTER
        const East = this.modulesLayout.get('east');
        if (East)
            html += this.addPanel('east', East);
        ...
        if (this.writer.isReadOnly || this.writer.isAnnotator) {
            const $fullscreenButton = $('<div class="fullscreenLink out">Fullscreen</div>').appendTo(this.$headerButtons);
        ...
        if (this.modulesLayout.get('south')) {
            ...
            innerLayoutConfig.south = {
                ...
                initClosed: true,
                //@ts-ignore
                activate: (event, ui) => $.layout.callbacks.resizeTabLayout(event, ui),
            };
        }
        //@ts-ignore
        ...
        this.modulesLayout.forEach((moduleConfig, region) => {
            ...
            const $region = this.$container?.find(`.ui-layout-${region}:not(.cwrcHeader):not(.cwrcFooter)`);
            if (!$region)
                return;
    ...
    isModuleAllowed(module) {
        return (!this.writer.isReadOnly || (this.writer.isReadOnly && !WRITE_ONLY_MODULES.includes(module.id)));
    }
    addPanel(panelRegion, panelConfig) {
        if (!panelConfig)
            return '';
        //single module
    ...
    initModule(module) {
        if (this.isModuleAllowed(module) === false)
            return null;
