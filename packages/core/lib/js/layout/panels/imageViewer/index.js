...
class ImageViewer {
    ...
    currentIndex = -1;
    ignoreScroll = false;
    constructor({ attribute, parentId, tag, writer }) {
        this.writer = writer;
    ...
    `);
        ...
        this.writer.event('documentLoaded').subscribe((success, body) => {
            if (!success)
                return;
        ...
        this.$parent.find('.currPage').on('keyup', function (event) {
            if (event.code === 'Enter') {
                let value = $(this).val();
                if (!value)
                    return;
        ...
        this.osd.addHandler('open-failed', (event) => {
            let msg = event.message;
            if (event.source.url === true)
                msg = `No URI found for @${this.attrName}.`;
            this.setMessage(msg);
        });
        this.osd.addHandler('reset-size', (event) => {
            if (event.contentFactor !== 0)
                return;
            this.setMessage(`No URI found for @${this.attrName}.`);
        });
        this.osd.addHandler('page', (event) => {
    ...
    loadPage(index, doScroll) {
        //out of bounds
        if (index < 0 || index >= this.$pageBreaks.length)
            return;
