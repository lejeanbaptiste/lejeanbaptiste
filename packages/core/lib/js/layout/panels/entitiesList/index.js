...
class EntitiesList {
    ...
    `);
        ...
        this.writer.event('entityRemoved').subscribe((entityId) => this.remove(entityId));
    ...
    acceptAll() {
        ...
        this.writer.entitiesManager.eachEntity((index, entity) => {
    ...
    acceptMatching(entityId) {
        const matches = this.getMatchesForEntity(entityId);
        this.acceptEntity(entityId);
        matches.forEach((entId) => this.acceptEntity(entId));
    }
    rejectMatching(entityId) {
        const matches = this.getMatchesForEntity(entityId);
        this.rejectEntity(entityId);
        matches.forEach((entId) => this.rejectEntity(entId));
    }
    rejectAll() {
    ...
    getSorting() {
        return this.$entities.find('select[name="sorting"]').val();
    }
    getMatchesForEntity(entityId) {
        const matches = [];
        const match = this.writer.entitiesManager.getEntity(entityId);
        this.writer.entitiesManager.eachEntity((index, entity) => {
            if (entity.getId() !== match.getId()) {
                if (JSON.stringify(entity.getAttributes()) === JSON.stringify(match.getAttributes()) &&
    ...
    getEntityView(entity) {
        ...
        if (lemmaHTML)
            propertiesHtmlArray.push(lemmaHTML);
        //URI
        const createUriHTML = () => {
            if (!uri)
                return undefined;
      ...
      `;
        ...
        if (uriHTML)
            propertiesHtmlArray.push(uriHTML);
        // attribute values
        let attAdded = false;
        ...
        for (const name in entityAttributes) {
          ...
          <li>
            <strong>${name}</strong>: <a href="${value}" target="_blank" rel="noopener">${value}</a>
                ...
                if (value === lemma)
                    continue; // don't duplicate lemma
                // if (!attAdded && nevAdded) propertiesHtmlArray.push('<li><hr /></li>');
    ...
    convertEntities() {
        ...
        this.writer.utilities
            .processArray(potentialEntities, (element) => {
    ...
    update() {
        ...
        let entitiesString = '';
        entities.forEach((entry) => (entitiesString += this.getEntityView(entry)));
        this.isConvert
