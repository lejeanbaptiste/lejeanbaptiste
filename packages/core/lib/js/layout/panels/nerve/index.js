...
function Nerve({ writer, parentId, nerveUrl }) {
  ...
  `);
    ...
    const requestNssi = async (document, context) => {
    ...
    const poolingNssiJob = async (jobId) => {
        const token = await writer.overmindActions.editor.getNssiToken();
    ...
    const getNssiResults = async ({ jobId, resultsUri }) => {
        const token = await writer.overmindActions.editor.getNssiToken();
    ...
    const requestLegacy = async (document, context) => {
        log.info(context);
    ...
    const postProcess = async (results) => {
        log.info(results);
        const li = writer.dialogManager.getDialog('loadingindicator');
