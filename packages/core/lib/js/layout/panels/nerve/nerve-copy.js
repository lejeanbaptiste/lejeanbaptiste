 ...
 */
function Nerve({ writer, parentId, nerveUrl }) {
    if (nerveUrl === undefined)
        log.error('Nerve: no nerveUrl specified!');
    const id = parentId;
    /**
     ...
     */
    let mergedEntities = {};
    let editDialog = null; // holder for initialized edit dialog
    let mergeDialog = null; // holder for initialized merge dialog
  ...
  `);
    ...
    $parent
        ...
        writer.dialogManager.confirm({
            title: 'Warning',
            msg: `
          <p>You are about to lose edits you've made to ${totalModified} Nerve-identified entit${totalModified > 1 ? 'ies' : 'y'}.</p>
    ...
    const run = async () => {
        ...
        if (results['http-response-status'] === 400) {
            log.warn(`The NERVE server returned an error. Bad request (possibly encode error): ${results.message}`);
            li.hide();
            writer.dialogManager.show('message', {
                title: 'Error',
        ...
        const doc = writer.utilities.stringToXML(results.document);
        if (!doc) {
    ...
    const getBasicXmlDocument = () => {
        ...
        const _nodeToStringArray = (currNode) => {
            ...
            //   array = ['', ''];
            // }
            const array = tag !== undefined ? [`<${tag}>`, `</${tag}>`] : ['', ''];
            return array;
        };
        const doBuild = (currentNode) => {
            const tags = _nodeToStringArray(currentNode);
            if (tags[0] === '<teiHeader>')
                return; // * skip `<teiheader>` when sending document to NSSI [NERVE]
            xmlString += tags[0];
    ...
    const buildContext = function () {
        ...
        //   );
        // }
        Object.entries(NerveToCWRCMappings).forEach(([nerveTypeName, entityType]) => {
            const tag = {
                name: sm.mapper.getParentTag(entityType),
                lemmaAttribute: sm.mapper.getMappingForProperty(entityType, 'lemma').replace('@', ''),
                linkAttribute: sm.mapper.getMappingForProperty(entityType, 'uri').replace('@', ''),
    ...
    const processNerveResponse = (document, context) => {
        const sm = writer.schemaManager;
        const entities = [];
        const tagsContext = {};
        ...
        const getOffsetFromParent = (parent, target) => {
            if (!parent)
                return 0;
            let offset = 0;
    ...
    const renderEntitiesList = () => {
        ...
        let entityHtml = '';
        getNerveEntities().forEach((ent, index) => {
    ...
    const getEntityView = function (entity, merge) {
        ...
        `
            : '';
        const actionsComponent = merge === true
            ? ''
            : `
    ...
    const getMergedEntityView = function (id, merge) {
        //@ts-ignore
        const entry = mergedEntities[id];
        const mergeOptionsComponents = merge !== true
            ? `
          <div class="actions">
            <div class="nav">
  ...
  `;
    ...
    const getEntityViewInfo = function (entity) {
    ...
    const getEntryForEntityId = (entityId) => writer.entitiesManager.getEntity(entityId);
    const removeEntityFromView = (id) => {
        $parent.find(`ul.entitiesList li[data-id=${id}]`).remove();
    };
    const selectRangeForEntity = (entry) => {
    ...
    const addEntityFromNerve = (entry) => {
    ...
    const acceptEntity = (entityId, removeFromView = true) => {
    ...
    const acceptAll = () => {
        ...
        writer.utilities
            .processArray(getNerveEntities(), (ent) => {
    ...
    const rejectEntity = (entityId, removeFromView = true) => {
    ...
    const editEntity = (entityId) => {
        if (editDialog === null) {
            ...
            editDialog.$el.on('save', (event, dialog) => {
    ...
    const mergeEntities = (id) => {
        if (mergeDialog === null) {
            //@ts-ignore
            mergeDialog = new MergeDialog(writer, $parent);
            mergeDialog.$el.on('merge', (event, entities, mergeEntry, lemma, uri) => {
                if (mergeEntry != null) {
        ...
        if (id !== undefined) {
            ...
            entities = entry.entityIds.map((entId) => writer.entitiesManager.getEntity(entId));
        }
        else {
    ...
    const unmergeEntities = (mergeId) => {
        //@ts-ignore
        delete mergedEntities[mergeId];
        renderEntitiesList();
    };
    const acceptMerged = (mergeId) => {
        //@ts-ignore
    ...
    const rejectMerged = (mergeId) => {
        //@ts-ignore
        const entry = mergedEntities[mergeId];
        entry.entityIds.forEach((entId) => rejectEntity(entId));
        //@ts-ignore
        delete mergedEntities[mergeId];
        removeEntityFromView(mergeId);
    };
...
const doLookup = (writer, query, type, callback) => {
...
const NerveEditDialog = (writer, parentEl) => {
  ...
  `).appendTo(parentEl);
    ...
    dialog.$el.on('beforeSave', (event, dialog) => {
        if (forceSave) {
            dialog.isValid = true;
        }
        else {
...
function MergeDialog(writer, parentEl) {
    let currEntities = [];
    let currEntry = null; // for editing
...
//# sourceMappingURL=nerve-copy.js.map