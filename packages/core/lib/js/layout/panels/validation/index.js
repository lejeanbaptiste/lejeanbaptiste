...
class Validation {
    id;
    writer;
    AUTO_VALIDATE_ONCHANGE_TIMER = 10000;
    autoValidateTimerActive = false;
    autoValidateTimer;
    progressBar; //typeof ProgressBar | null = null;
    ...
    `);
      ...
      `;
        ...
        this.writer.event('documentValidating').subscribe((partDone) => {
     ...
     */
    showValidationResult({ valid, errors }) {
        const list = $(`#${this.id} > div.validationList`);
        list.empty();
        if (valid) {
    ...
    createErrorMessage({ type, msg, target, element }) {
        switch (type) {
    ...
    createErrorMessageComponent(data) {
        const { type } = data;
    ...
    async createDocumentationComponent($item) {
    ...
    async getPossible({ type, target, element, }) {
        switch (type) {
            case 'ElementNameError':
