...
class StructureTree {
    ...
    constructor({ parentId, writer }) {
        ...
        $(`#${parentId}`).append(`<div class="moduleParent">
        ...
        if (this.writer.isReadOnly !== true)
            plugins.push('dnd');
        this.$tree.jstree({
            plugins,
            core: {
                ...
                check_callback: (operation, node, node_parent, node_position, more) => {
                    if (this.writer.editor.readonly &&
        ...
        this.$tree.on('contextmenu', (event) => {
            ...
            if ($target.length !== 1)
                return;
            const selectedIds = this.currentlySelectedNodes; // store selected nodes before highlighting
        ...
        this.$tree.on('copy_node.jstree', (event, data) => this.onDragDrop(data, true));
        this.$tree.on('move_node.jstree', (event, data) => this.onDragDrop(data, false));
        this.$tree.on('keydown.jstree', (event) => {
            //log.info(event);
        });
        this.$tree.on('ready.jstree', (event, data) => {
        ...
        this.writer.event('nodeChanged').subscribe((currentNode) => {
        ...
        this.writer.event('writerKeydown').subscribe((event) => {
            if (this.currentlySelectedNodes.length <= 0)
                return;
        ...
        this.writer.event('writerKeyup').subscribe((event) => {
        ...
        this.writer.event('entityAdded').subscribe((entityId) => this.update());
        this.writer.event('entityRemoved').subscribe((entityId) => this.update());
        this.writer.event('entityFocused').subscribe((entityId) => {
            // if (!this.ignoreSelect) {
        ...
        this.writer.event('entityPasted').subscribe((entityId) => this.update());
        this.writer.event('tagAdded').subscribe((tag) => this.update());
        this.writer.event('tagEdited').subscribe((tag) => this.update());
        this.writer.event('tagRemoved').subscribe((tag) => this.update());
        this.writer.event('tagContentsRemoved').subscribe((tagId) => this.update());
        this.writer.event('tagSelected').subscribe((tagId) => {
    ...
    update() {
        ...
        this.clear();
        let rootNode = $(`[_tag="${this.writer.schemaManager.getRoot()}"]`, this.writer.editor.getBody());
        ...
        if (rootData !== null) {
            ...
            if (this.writer.isReadOnly) {
                ...
                const removeParentProperty = (nodeData) => {
                    delete nodeData.parent;
                    if (nodeData.children) {
                        nodeData.children.forEach((child) => removeParentProperty(child));
                    }
                };
    ...
    expandParentsForNode(node) {
        ...
        for (const parentId of parents) {
            ...
            if (!isOpen)
                this.$tree.jstree('open_node', parentNode, null, false);
        }
        return headerId;
    }
    /**
     ...
     */
    doSelectNode = ($node, selectContents, multiselect, external) => {
    ...
    processNode(node, level) {
        ...
        if (this.writer.isReadOnly === false && node.attr('_entity')) {
            ...
            if (this.writer.isReadOnly === false ||
     ...
     */
    doUpdate(children, nodeParent, level, lastEntry) {
        const _this = this;
    ...
    doConditionalSelect = (node, event) => {
        //@ts-ignore
    ...
    onNodeDeselect = (event, data) => {
        if (data !== undefined) {
            ...
            if (index !== -1)
                this.currentlySelectedNodes.splice(index, 1);
        }
        else {
            // clear everything
            this.removeCustomClasses();
    ...
    onDragDrop(data, isCopy) {
        ...
        if (isCopy)
            this.writer.tagger.processNewContent(dragNodeEditor[0]);
