...
const useSettings = () => {
  ...
  return {
    ...
    changeEditorMode: ({ mode, isUndo }: { mode: string; isUndo?: boolean }) => {
    ...
    changeAnnotationMode: ({ mode, isUndo }: { mode: number; isUndo?: boolean }) => {
    ...
    schemaShouldChange: async (
      ...
      const writer = window.writer;
      if (!writer) return [false, { type: 'error', text: 'writer not found' }];
    ...
    changeSchema: ({ schemaId, isUndo }: { schemaId: string; isUndo?: boolean }) => {
