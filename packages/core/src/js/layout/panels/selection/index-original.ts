...
class Selection {
  ...
  `);
    ...
    this.writer.event('nodeChanged').subscribe(() => {
      if (!this.showingFullDoc) this.updateView();
    });
  ...
  private async updateView(useDoc: boolean = false, initialSetup: boolean = false) {
    if (!this.enabled) return;

    const timestamp = new Date().getTime();
    const timeDiff = timestamp - this.lastUpdate; // track to avoid double update on nodeChanged/tagSelected combo
    if ((!this.$prismContainer.is(':visible') || timeDiff < 250) && !initialSetup) return;
  ...
  private showString(xmlString: string) {
