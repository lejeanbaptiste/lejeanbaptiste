...
or **BDRC** page to Le Jean-Baptiste.
...
Download the extension zips from the [same GitHub release](https://github.com/lejeanbaptiste/lejeanbaptiste/releases/latest) as the desktop app (`ljb-browser-extension-chromium-*.zip` and `ljb-browser-extension-firefox-*.zip`). Step-by-step instructions for every browser and OS are in the main [readme.md](../../readme.md#browser-extension-corpus-import).

## Install from a git checkout (developers)

### Chrome / Brave / Edge

1. Start **Le Jean-Baptiste** once (writes the native-messaging manifests).
...
Starting Le Jean-Baptiste registers the native host `org.lejeanbaptiste.import`
...
`ljb-corpus-import@lejeanbaptiste.org`. On Windows the host is launched through
a generated `ljb-browser-host.bat` (the browser can only spawn `.exe`/`.bat`/
...
uninstalling LJB leaves the stale entry behind (harmless — it just fails to
connect).
