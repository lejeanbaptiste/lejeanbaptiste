; Override electron-builder's extractAppPackage.nsh (same filename under
; buildResources is searched first). Fixes two ARM64-only installer bugs:
;
; 1. identify_package only selects ARM64 when IsNativeARM64 is true. Some ARM
;    VMs / environments fail that check, leaving $packageArch empty so nothing
;    is extracted (installer finishes with no .exe — only an uninstaller).
; 2. For an ARM64-only build there is only one embedded archive; always use it.
;
; Keep the rest of this file aligned with app-builder-lib's template aside from
; the identify_package fallback.
...
!macro identify_package
  ...
  !ifdef APP_ARM64
    ...
    ${endif}
    ; ARM64-only installer: if detection failed, still extract the only package.
    !ifndef APP_64
      !ifndef APP_32
        ${if} $packageArch == ""
          StrCpy $packageArch "ARM64"
