...
const timestamp = (item: XmlElement): string => {
  const changed = childElements(item).find(
    (child) => localName(child) === 'note' && child.getAttribute('type') === 'ljb-changed',
  );
...
export function importEntitiesXml(
  ...
  if (localName(root.documentElement) !== 'TEI' || !databaseId) {
    throw new Error('Not a valid Le Jean-Baptiste entity database XML document.');
  }
    ...
    for (const list of childElements(standOff)) {
      ...
      for (const item of childElements(list)) {
        ...
        for (const child of childElements(item)) {
          ...
          if (childName === 'idno' && child.getAttribute('type') === 'ljb-central') {
          ...
          if (childName === 'idno' && child.getAttribute('type') !== 'ljb-entity-database') {
            db.prepare(
          ...
          if (childName === 'note' && child.getAttribute('type') === 'ljb-changed') continue;
...
function entityXml(db: DatabaseSync, entity: Record<string, unknown>): string {
  ...
  for (const row of rows(
    ...
    parts.push(
      `<idno type="ljb-central" subtype="${attrEscape(String(row.user_stable_id))}">${xmlEscape(String(row.central_entity_id))}</idno>`,
    );
  }
  for (const row of rows(
    db,
  ...
  parts.push(`<note type="ljb-changed" when="${attrEscape(changed)}"/>`);
...
/**
 * Strip local ids, `ljb-central` mappings, and volatile `ljb-changed` stamps so
