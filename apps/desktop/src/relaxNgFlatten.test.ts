...
const WRAPPER = `<?xml version="1.0"?><grammar xmlns="http://relaxng.org/ns/structure/1.0" ns="http://www.tei-c.org/ns/1.0"><include href="tei_all.tei.rng"><define name="date"><element name="date"><ref name="grognard.sanmiao.era"/></element></define></include><define name="grognard.sanmiao.era"><element name="era"><text/></element></define></grammar>`;
...
describe('flattenRelaxNgGrammar', () => {
  it('inlines includes, applies override defines, and keeps sibling helper defines', () => {
    ...
    expect(flat).toContain('grognard.sanmiao.era');
    ...
    const hasSanmiaoRef = Array.from(refs).some(
      (ref: Element) => ref.getAttribute('name') === 'grognard.sanmiao.era',
    );
