...
const PAGES = [
  { id: 'EP_1', seq: 1, s: 0, e: 10 },
  { id: 'EP_2', seq: 2, s: 10, e: 25 },
];
...
test('fetchChunks: windows the volume, dedupes, sorts, collects pages + image group', async () => {
  const { fetchImpl, calls } = makeFetch();
  const { chunks, pages, imageGroupId } = await fetchChunks(UT, { fetchImpl, windowSize: 8 });
...
test('pipeline: importEtext → etextToBodyXml → one <pb> per folio, <lb> per line', async () => {
  ...
  assert.match(bodyXml, /<pb n="1a" facs="[^"]+f1\.jpg[^"]*"\/>AAAAAAAAAA/);
  assert.match(bodyXml, /<pb n="1b" facs="[^"]+f2\.jpg[^"]*"\/>BBBBB<lb\/>BBBBBBBBB/);
